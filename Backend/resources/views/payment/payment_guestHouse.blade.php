<div class="credit-card w-full sm:w-auto shadow-lg mx-auto rounded-xl bg-white pr-4 pl-4">
    <div class="w-full h-8 flex justify-end">
        <button id="closeModalPayment" class="text-gray-500 hover:text-gray-700 m-4">
        ...
        <div>
            ...
            <div class="my-3">
                <input
                    ...
                    placeholder="Card holder" maxlength="22" type="text" id="customer_name" name="customer_name"
                    required>
            </div>
            <div class="my-3">
                <div id="card-element"
...
<script>
    ...
    let amoutPaid = document.getElementById('amoutPaid');
    let guestHouseId = null;

    cardPayments.forEach(cardPayment => {
        cardPayment.addEventListener('click', () => {
            modalPayment.classList.remove('hidden');
            guestHouseId = cardPayment.dataset.payment;
            amoutPaid.textContent = `$${cardPayment.dataset.amount}`;
        });
    });
    ...
    form.addEventListener('submit', function(event) {
        ...
        fetch("/admin/process-payment", { // Add a unique identifier to prevent caching
                method: "POST",
                headers: {
    ...
    // Pushing to activate quest house by updating new data of id guest house and create the table of the payment
    function paymentPaid() {
        $.ajax({
            type: 'POST',
            url: '{{ route('admin.paid.guestHouse') }}',
            data: {
                'guestHouseId': guestHouseId,
                'amount': amoutPaid.textContent.slice(1, amoutPaid.textContent.length),
                '_token': '{{ csrf_token() }}'
            },
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
    ...
    // popup modal for create guest house
